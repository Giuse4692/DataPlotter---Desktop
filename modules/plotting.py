...
def calculate_and_plot_intersections(fig, df, x_axis_name, ref_type, val1, val2, show_points, show_table):
    ...
    if ref_type == 'x_const':
        ...
        for trace in fig.data:
            # NOTA: Questo controllerà le intersezioni su TUTTE le tracce, 
            # indipendentemente dal fatto che siano sull'asse Y sinistro o destro.
    ...
    if intersection_results:
        ...
        if show_points:
            fig.add_trace(go.Scatter(
                ...
                mode='markers',
                name='Intersezioni',
                # NOTA: I punti di intersezione sono plottati sull'asse Y primario (sinistro)
                yaxis='y1', 
...
def show_plotting_ui(df):
    ...
    # NUOVA MODIFICA: Variabile per grafici 2D
    ...
    # NUOVA MODIFICA: Etichetta e variabile per Asse Y Sinistro
    y_axes_left_label = "Assi Y (Sinistra)" if is_2d else "Assi Y (Curve 2D) / Asse Y (Curve 3D)"
    ...
    # NUOVA MODIFICA: Selettore per Asse Y Destro (solo per 2D)
    y_axes_right = []
    if is_2d:
        # Rimuovi le colonne già selezionate a sinistra da quelle disponibili
    ...
    # Logica per 3D (usa y_axes_left come riferimento)
    y_axis_single = y_axes_left[0] if y_axes_left else column_list[1] if len(column_list) > 1 else column_list[0]
    ...
    # NUOVA MODIFICA: Titolo e Label aggiornati per entrambi gli assi
    all_y_axes = y_axes_left + y_axes_right
    default_title = f"{', '.join(all_y_axes)} vs {x_axis}" if len(all_y_axes) > 1 and not is_3d else f"{y_axis_single} vs {x_axis}"
    ...
    # Etichetta Asse Y Sinistro
    y_label_left_default = ", ".join(y_axes_left) if len(y_axes_left) > 1 and not is_3d else y_axis_single
    y_label_left = st.sidebar.text_input("Etichetta Asse Y (Sinistra)", y_label_left_default)
    
    # NUOVA MODIFICA: Etichetta Asse Y Destro (solo se usato)
    y_label_right = ""
    if y_axes_right:
        y_label_right_default = ", ".join(y_axes_right)
        y_label_right = st.sidebar.text_input("Etichetta Asse Y (Destra)", y_label_right_default)

    if z_axis:
        z_label = st.sidebar.text_input("Etichetta Asse Z", z_axis)
    else:
        z_label = "Z" # Assicurati che z_label sia definito
    ...
    # NUOVA MODIFICA: Checkbox per asse sinistro
    log_y_left = st.sidebar.checkbox("Scala Logaritmica Asse Y (Sinistra)", key="log_y_left")
    
    # NUOVA MODIFICA: Checkbox per asse destro (solo se usato)
    log_y_right = False
    if y_axes_right:
    ...
    st.sidebar.header("5. Linee di riferimento e punti")
    ...
    try:
        # ----------------------------------------------------
        # LOGICA 2D: TRACCE MULTIPLE (CON DOPPIO ASSE)
        # ----------------------------------------------------
        if plot_type in ["Linea 2D", "Scatter 2D"]:
            fig = go.Figure() 
            mode = 'lines' if plot_type == "Linea 2D" else 'markers'
            
            # NUOVA MODIFICA: Loop per le tracce dell'ASSE SINISTRO (y1)
            ...
            for y_col in y_axes_right:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    y=df[y_col],
                    mode=mode,
                    name=f"{y_col} (Destra)", # Aggiungi suffisso per chiarezza in legenda
                    yaxis='y2', # !!! Assegna all'asse y2 !!!
                    line=dict(color=color, width=width) if mode == 'lines' else None,
                    marker=dict(color=color, size=width) if mode == 'markers' else None
                ))
            
            # --- Gestione Annotazioni e Intersezioni ---
            
            # Aggiunta traccia PUNTI DI ANNOTAZIONE 2D (plottati su asse y1)
            annotation_trace = au.get_annotations_trace(custom_points, is_3d=False) 
            if annotation_trace:
                fig.add_trace(annotation_trace) # Di default va su y1
            ...
            if ref_type and val1 is not None:
                # La linea di riferimento è plottata sull'asse y1
                ...
                # Il calcolo delle intersezioni ora funziona per TUTTE le tracce (y1 e y2)
                calculate_and_plot_intersections(fig, df, x_axis, ref_type, val1, val2, show_points, show_table)


        # ----------------------------------------------------
        # LOGICA 3D (Invariata, usa y_axes_left per y_axis_single)
        # ----------------------------------------------------
            ...
            if not color_axis or len(y_axes_left) > 1:
                 fig = px.scatter_3d(
        ...
        # ----------------------------------------------------
        # AGGIORNAMENTO LAYOUT (MODIFICATO PER DOPPIO ASSE)
        # ----------------------------------------------------
        if fig: 
            # --- Layout (Generale) ---
            fig.update_layout(
            ...
            if is_3d:
                fig.update_layout(scene=dict(
                    xaxis_title=x_label,
                    yaxis_title=y_label_left, # Usa y_label_left
                    zaxis_title=z_label
                ))
            else:
                # NUOVA MODIFICA: Configurazione layout per 2D
                x_type = "log" if log_x else "linear"
                y_type_left = "log" if log_y_left else "linear"
                
                # Dizionario di base per il layout
                layout_update_dict = {
                    'xaxis_title': x_label,
                    'yaxis_title': y_label_left, # Asse Y primario (sinistro)
                    'xaxis': dict(type=x_type),
                    'yaxis': dict(type=y_type_left)
                }
        ...
        st.warning("Seleziona almeno una colonna per l'Asse Y (Sinistra).")
